...
4. علامات الترقيم: احذف جميع علامات الترقيم من نهايات الجمل، باستثناء علامة الاستفهام. وإذا أبقيت علامة الاستفهام، يجب أن تكون علامة الاستفهام العربية الصحيحة (؟) وليس الانجليزية (?).
...
{formatted_scribe_text}"""
    ...
    if len(corrected_lines) == len(corrected_chunks):
        ...
        for i, new_line in enumerate(corrected_lines):
            corrected_chunks[i]["text"] = new_line
